...
type EditedWeather = Partial<
  Pick<CityWeather, "temperatureC" | "humidity" | "windKph" | "airQuality" | "precipitation" | "uvIndex">
>;
...
type WeatherCondition = "Sunny" | "Clear" | "Partly Cloudy" | "Cloudy" | "Rain" | "Storm" | "Snow" | "Windy";
...
function getWeatherNarrative(city: CityWeather) {
  ...
  if (city.condition === "Sunny" && city.uvIndex >= 8)
    return "Heat and UV mitigation matter more than rain protection.";
...
function getComfortScore(city: CityWeather) {
  return clampPercent(
    100 - Math.abs(city.temperatureC - 22) * 3 - city.humidity * 0.22 - city.windKph * 0.45 - city.precipitation * 0.3,
  );
}
...
function getAstronomyFacts(city: CityWeather) {
  ...
  const moonIllumination = clampPercent(
    city.moonPhase.includes("Full")
      ? 100
      : city.moonPhase.includes("Quarter")
        ? 52
        : city.moonPhase.includes("Gibbous")
          ? 74
          : 28,
  );
  return {
...
function getMomentRecommendation(city: CityWeather, moment: (typeof travelMoments)[number]) {
  const baseTemp =
    city.temperatureC + (moment === "Midday" ? 3 : moment === "Night" ? -4 : moment === "Evening" ? -1 : 0);
  ...
  const items = [
    {
      ...
      note:
        city.precipitation > 55 ? "Intervals indoors or on sheltered loops." : "Outdoor pacing window remains usable.",
    },
    {
      label: "Walking",
    ...
    },
    {
      ...
      note:
        city.visibilityKm >= 10
          ? "Good scene depth and skyline definition."
          : "Lower contrast and softer distance detail.",
    },
    {
      label: "Beach",
      ...
      note:
        city.id === "dubai"
          ? "Watch heat load even when conditions look calm."
          : "Only viable in warmer, lower-wind windows.",
    },
    {
      label: "Commute",
      ...
      note:
        city.condition === "Storm"
          ? "Transit reliability is the main constraint."
          : "General city movement is manageable.",
    },
    {
      ...
      note:
        city.alerts.length > 0
          ? "Build buffer time around active alerts."
          : "Transfer confidence stays relatively stable.",
    },
  ];
...
function getRouteRisk(city: CityWeather) {
  return [
    ...
    },
    {
      ...
      note:
        city.alerts.length > 0
          ? "Keep margin for traffic/weather variability."
          : "Arrival routing is relatively stable.",
    },
  ];
}
...
function getHistoryStates(city: CityWeather) {
  return [
    ...
    {
      label: "Monthly norm",
      deltaTemp: -getClimateComparison(city).tempDelta,
      deltaRain: -getClimateComparison(city).rainDelta,
      deltaWind: -getClimateComparison(city).windDelta,
    },
  ];
}
  ...
  switch (condition) {
    ...
    case "Clear":
      return {
        background:
          [
            "radial-gradient(circle at top left, rgba(255, 201, 102, 0.34), transparent 32%), ",
            "radial-gradient(circle at top right, rgba(255, 127, 80, 0.2), transparent 26%), ",
            "linear-gradient(135deg, #241304 0%, #3b2410 48%, #22140d 100%)",
          ].join(""),
      };
    case "Storm":
      return {
        background:
          [
            "radial-gradient(circle at top left, rgba(126, 116, 255, 0.28), transparent 32%), ",
            "radial-gradient(circle at top right, rgba(90, 174, 255, 0.18), transparent 28%), ",
            "linear-gradient(135deg, #07101f 0%, #17213d 52%, #0b1524 100%)",
          ].join(""),
      };
    case "Snow":
      return {
        background:
          [
            "radial-gradient(circle at top left, rgba(205, 236, 255, 0.28), transparent 32%), ",
            "radial-gradient(circle at top right, rgba(121, 170, 255, 0.16), transparent 28%), ",
            "linear-gradient(135deg, #0c1621 0%, #183247 52%, #10202c 100%)",
          ].join(""),
      };
    case "Rain":
      return {
        background:
          [
            "radial-gradient(circle at top left, rgba(82, 157, 255, 0.3), transparent 32%), ",
            "radial-gradient(circle at top right, rgba(68, 227, 196, 0.16), transparent 28%), ",
            "linear-gradient(135deg, #06131f 0%, #12314a 52%, #0c1725 100%)",
          ].join(""),
      };
    default:
      return {
...
function getHealthSignals(city: CityWeather, profile: UserProfile) {
  ...
  return [
    { label: "Sleep comfort", score: Math.round(sleep), note: "Night recovery and bedroom ventilation friendliness." },
    {
      ...
      score: Math.round(respiratory),
      note: "Air quality and moisture pressure on breathing comfort.",
    },
    {
      label: "Migraine risk",
      score: Math.round(migraine),
      note: "Pressure and wind stability against sensitivity spikes.",
    },
    { label: "Skin balance", score: Math.round(skin), note: "Humidity and UV stress for exposed skin." },
    {
      label: "Thermal tolerance",
      score: Math.round(thermal),
      note: "How well the current city fits your profile thresholds.",
    },
  ];
}
  ...
  return [
    {
      ...
      note:
        city.temperatureC >= 28 ? "Carry water before extended outdoor movement." : "Short sessions stay manageable.",
    },
    {
      label: "Outer shell",
      status: city.precipitation >= 40 || city.condition === "Snow" ? "Required" : "Optional",
      note:
        city.precipitation >= 40 ? "Rain protection improves trip reliability." : "A shell is not critical right now.",
    },
    {
    ...
    },
    {
      ...
      note:
        city.uvIndex >= 6 ? "SPF and eyewear should be part of the default kit." : "UV is not the dominant constraint.",
    },
  ];
}
...
function getWeeklyStrategy(city: CityWeather) {
  return city.weekly.map((entry) => {
    ...
    const label = comfort >= 72 ? "Prime day" : comfort >= 48 ? "Manageable" : "Protected plan";
    const note =
      entry.rainChance >= 60
...
function getScenarioInsights(city: CityWeather, purpose: TripPurpose, profile: UserProfile) {
  ...
  const resilience = clampPercent((purposeBias + getProfileBias(city, profile) + getHydrationScore(city)) / 3);
...
function getMonthlyLedger(city: CityWeather) {
  ...
  return baselines.map(([month, tempC, rain]) => ({
    ...
    label: rain >= 65 ? "Wet leaning" : tempC >= 28 ? "Hot leaning" : tempC <= 2 ? "Cold leaning" : "Balanced",
  }));
}
...
function getHabitWindows(city: CityWeather, profile: UserProfile) {
  return [
    ...
    },
    {
      ...
      score: clampPercent(
        Math.round((getHealthSignals(city, profile)[0].score + getHealthSignals(city, profile)[3].score) / 2),
      ),
...
function getPhotoPlan(city: CityWeather) {
  return [
    ...
    },
    {
      ...
      note:
        city.precipitation >= 40
          ? "Reflections improve mood but reduce gear safety."
          : "Street work stays straightforward.",
    },
    {
      frame: "Waterfront",
    ...
    },
    {
      ...
      note:
        city.condition === "Rain"
          ? "Wet surfaces can improve contrast and highlights."
          : "Mobility and visibility are the main levers.",
    },
  ];
}
  ...
  return [
    {
      ...
      note:
        city.temperatureC >= 28
          ? "Front-load fluids before the late afternoon drop."
          : "Standard rehydration pacing is enough.",
    },
    {
      label: "Sleep setup",
      ...
      note:
        city.humidity >= 70
          ? "Ventilation matters more than insulation tonight."
          : "Keep a stable room temperature and reduce late heat exposure.",
    },
    {
    ...
    },
    {
      ...
      note:
        city.precipitation >= 45
          ? "Dry footwear and friction prevention are a priority."
          : "Standard recovery is enough after city mileage.",
    },
  ];
}
  ...
  return [
    {
      ...
      score: clampPercent(
        Math.round((getComfortScore(city) + getCommuteScore(city) + getProfileBias(city, profile)) / 3),
      ),
...
function getCityDossier(city: CityWeather) {
  return [
    ...
    { label: "Station elevation", value: `${city.seaLevelM} m` },
    {
      ...
      value:
        city.visibilityKm >= 12 ? "Long-range clear" : city.visibilityKm >= 8 ? "Moderate range" : "Compressed range",
    },
  ];
}
...
function getDecisionMatrix(city: CityWeather, profile: UserProfile, purpose: TripPurpose) {
  return [
    ...
    },
    {
      ...
      score: clampPercent(
        Math.round((getScenarioInsights(city, purpose, profile).resilience + getTravelDelayBuffer(city)) / 2),
      ),
...
function getDaypartOperations(city: CityWeather) {
  return [
    {
      label: "Early morning",
      score: clampPercent(Math.round(getComfortScore(city) + 6)),
      note: "Usually the lowest-exposure window.",
    },
    {
      label: "Late morning",
      score: clampPercent(Math.round(getCommuteScore(city) + 2)),
      note: "Good for scheduled movement and errands.",
    },
    {
      label: "Afternoon",
      score: clampPercent(Math.round(getHydrationScore(city))),
      note: "Heat and UV load matter more here.",
    },
    {
      label: "Evening",
      score: clampPercent(Math.round((getPhotoScore(city) + getComfortScore(city)) / 2)),
      note: "Best balance of atmosphere and walkability.",
    },
  ];
}
  ...
  return [
    {
      ...
      note:
        city.condition === "Storm" ? "Protected transfers matter most." : "Fastest place for reliable movement blocks.",
    },
    {
      label: "Cultural core",
      note:
        city.precipitation >= 45
          ? "Indoor anchors make it a strong fallback zone."
          : "Good for flexible walking loops and long stays.",
    },
    {
      label: "Waterfront",
      note:
        city.windKph >= 24 ? "Use selectively because of wind exposure." : "Best for open visuals and sunset pacing.",
    },
    {
      label: "Residential edges",
      note:
        city.airQuality >= 75
          ? "Prefer shorter stays if air quality is the limiting factor."
          : "Often calmer and better for recovery pacing.",
    },
  ];
}
...
function getQuickWins(city: CityWeather) {
  return [
    ...
    },
    {
      ...
      note: city.alerts.length > 0 ? "Transit-heavy route with indoor stops" : "Flexible mixed indoor-outdoor route",
    },
  ];
}
...
function WeatherGlyph({ condition, large = false }: { condition: WeatherCondition; large?: boolean }) {
    ...
    return (
      <svg viewBox="0 0 64 64" className={className} aria-hidden="true">
        ...
        {[...Array(8)].map((_, index) => {
          ...
          return (
            <line key={angle} x1={x1} y1={y1} x2={x2} y2={y2} stroke={accent} strokeWidth="4" strokeLinecap="round" />
          );
        })}
      </svg>
    );
  ...
  if (condition === "Storm") {
    return (
      <svg viewBox="0 0 64 64" className={className} aria-hidden="true">
        <path
          d={
            "M18 40c-7 0-12-5-12-11s5-11 12-11c2 0 4 .4 5.8 1.2C26.6 13.7 32.1 10 38.5 10" +
            "c9 0 16.4 7.1 16.4 16 0 8-6 14-14 14H18Z"
          }
          fill="rgba(214,224,239,0.85)"
        />
  ...
  if (condition === "Rain") {
    return (
      <svg viewBox="0 0 64 64" className={className} aria-hidden="true">
        <path
          d={
            "M17 38c-7 0-12-5-12-11s5-11 12-11c1.5 0 3 .2 4.3.7C24 10.8 29.3 8 35.3 8" +
            "c8.4 0 15.2 6.6 15.2 14.8 0 8.3-6.8 15.2-15.2 15.2H17Z"
          }
          fill="rgba(214,224,239,0.85)"
        />
  ...
  if (condition === "Windy") {
    return (
      <svg viewBox="0 0 64 64" className={className} aria-hidden="true">
        <path
          d="M12 25h24c4 0 7-3 7-7 0-3.3-2.7-6-6-6-2.9 0-5.3 2-5.9 4.7"
          fill="none"
          stroke={accent}
          strokeWidth="4"
          strokeLinecap="round"
        />
        <path
          d="M10 34h38c4 0 6 2.7 6 6s-3 7-7 7c-2.8 0-5.2-1.8-6-4.3"
          fill="none"
          stroke={accent}
          strokeWidth="4"
          strokeLinecap="round"
  ...
  return (
    <svg viewBox="0 0 64 64" className={className} aria-hidden="true">
      ...
      <path
        d={
          "M22 42c-7 0-12-5-12-11s5-11 12-11c1.8 0 3.6.4 5.2 1.1C29.9 16.1 35.3 13 41.5 13 " +
          "50.1 13 57 19.6 57 28.1 57 35.7 51 42 43 42H22Z"
        }
        fill="rgba(214,224,239,0.88)"
      />
    </svg>
  );
}
...
function App() {
  ...
  const [favorites, setFavorites] = useState<string[]>(() =>
    JSON.parse(localStorage.getItem("weather-favorites") ?? '["tokyo","dubai"]'),
  );
  const [profile, setProfile] = useState<UserProfile>(() =>
    JSON.parse(localStorage.getItem("weather-profile") ?? JSON.stringify(defaultProfile)),
  );
  const [showCommandPalette, setShowCommandPalette] = useState(false);
  const [showProfilePanel, setShowProfilePanel] = useState(false);
  const [showEditorPanel, setShowEditorPanel] = useState(false);
  const [historyIndex, setHistoryIndex] = useState(0);
  const [editedWeather, setEditedWeather] = useState<Record<string, EditedWeather>>(() =>
    JSON.parse(localStorage.getItem("weather-edits") ?? "{}"),
  );
  const [preset, setPreset] = useState<DashboardPreset>(
    () => (localStorage.getItem("weather-preset") as DashboardPreset) || "overview",
  );
  const [notificationFilter, setNotificationFilter] = useState<NotificationType | "all">("all");
  const [journalDraft, setJournalDraft] = useState("");
  const [journal, setJournal] = useState<Record<string, string[]>>(() =>
    JSON.parse(localStorage.getItem("weather-journal") ?? "{}"),
  );
  const [savedViews, setSavedViews] = useState<SavedView[]>(() =>
    JSON.parse(localStorage.getItem("weather-saved-views") ?? "[]"),
  );
  const [settings, setSettings] = useState<AppSettings>(() =>
    JSON.parse(localStorage.getItem("weather-settings") ?? '{"motion":true,"denseCards":false,"advisoryMode":true}'),
  );
  const [tripPurpose, setTripPurpose] = useState<TripPurpose>(
    () => (localStorage.getItem("weather-trip-purpose") as TripPurpose) || "city-break",
  );
  ...
  const [tripTasks, setTripTasks] = useState<Record<string, TripTask[]>>(() =>
    JSON.parse(localStorage.getItem("weather-trip-tasks") ?? "{}"),
  );
  ...
  const data = useMemo(
    () => weatherData.map((item) => mergeCityWeather(item, editedWeather[item.id])),
    [editedWeather],
  );
  ...
  const plannerMoments = useMemo(
    () => travelMoments.map((moment) => ({ moment, ...getMomentRecommendation(city, moment) })),
    [city],
  );
  ...
  const filteredNotifications = useMemo(
    () =>
      ...
      ).filter((item) => notificationPrefs[item.type]),
  ...
  const travelPlanner = useMemo(
    () => [
      {
        label: "06:00-09:00",
        score: Math.round((getComfortScore(city) + profileBias) / 2),
        note: "Best for early transfers and lower thermal load.",
      },
      {
        label: "09:00-12:00",
        score: Math.round((getCommuteScore(city) + getComfortScore(city)) / 2),
        note: "Balanced window for city movement and errands.",
      },
      {
        label: "12:00-15:00",
        score: Math.round((getHydrationScore(city) + getComfortScore(city)) / 2),
        note: "Watch direct exposure, especially in high UV or heat.",
      },
      {
        label: "15:00-18:00",
        score: Math.round((getPhotoScore(city) + getComfortScore(city)) / 2),
        note: "Good candidate for outdoor stops and visual scenes.",
      },
      {
        label: "18:00-22:00",
        score: Math.round((getPhotoScore(city) + getCommuteScore(city)) / 2),
        note: "Useful for dining, skyline windows and late transfers.",
      },
    ],
    [city, profileBias],
  );
  ...
  function toggleFavorite(cityId: string) {
    setFavorites((current) =>
      current.includes(cityId) ? current.filter((item) => item !== cityId) : [...current, cityId],
    );
  }

  function updateCityField<K extends keyof EditedWeather>(
    cityId: string,
    field: K,
    value: NonNullable<EditedWeather[K]>,
  ) {
  ...
  function addTripTask() {
    ...
    setTripTasks((current) => ({
      ...current,
      [city.id]: [{ id: `${city.id}-${Date.now()}`, text: trimmed, done: false }, ...(current[city.id] ?? [])].slice(
        0,
        10,
      ),
    }));
        ...
        <section className="topbar">
          <div className="branding">
            ...
            <p className="subtitle">
              Large single-file React project with a dense interface, mock locations, unit switching, persisted state,
              search, and rich weather cards without splitting logic across extra files.
            </p>
          ...
          <div className="control-row">
            ...
            <div className="unit-toggle" aria-label="Temperature units">
              {(["C", "F"] as Unit[]).map((nextUnit) => (
                ...
                >
                  {nextUnit}°<span className="unit-small">{unitLabels[nextUnit]}</span>
                </button>
              ))}
            </div>
          </div>
          ...
          {data.map((item) => (
            ...
            >
              ...
              <span className="metric-subtle">
                {favorites.includes(item.id) ? "Pinned to quick access" : "Add to favorites"}
              </span>
            </button>
          ))}
        </div>
        ...
        <div className="preset-row">
          {(["overview", "travel", "fitness", "photo"] as DashboardPreset[]).map((item) => (
            <button
              key={item}
              type="button"
              className={`preset-card ${preset === item ? "active" : ""}`}
              onClick={() => setPreset(item)}
            >
              <strong>{item}</strong>
              <span className="metric-subtle">
                {item === preset ? getPresetNarrative(item) : "Switch dashboard emphasis"}
              </span>
            </button>
          ))}
        </div>
        ...
        <div className="filter-row">
          {(["city-break", "business", "outdoor", "nightlife"] as TripPurpose[]).map((item) => (
            <button
              key={item}
              type="button"
              className={`filter-chip ${tripPurpose === item ? "active" : ""}`}
              onClick={() => setTripPurpose(item)}
            >
              {item}
            </button>
          ))}
        ...
        <section className="layout">
            ...
            <article className="glass-card timeline-card">
              ...
              <div className="deep-grid">
                ...
                <div className="advisor-note">
                  ...
                  {selectedTimeline.rainChance < 25 ? "outdoor transitions" : "short protected routes"} and works best
                  if you plan around {selectedTimeline.label.toLowerCase()}.
                </div>
              </div>
            </article>
              ...
              <div className="checklist-grid">
                {tripChecklist.map((entry) => (
                  <div key={entry.label} className="checklist-card">
                    ...
                    <p className="metric-subtle" style={{ marginTop: 10 }}>
                      {entry.note}
                    </p>
                  </div>
                ))}
              ...
              <div className="toggle-row">
                {historyStates.map((entry, index) => (
                  <button
                    key={entry.label}
                    type="button"
                    className={index === historyIndex ? "action-button" : "ghost-button"}
              ...
              <div className="history-grid">
                ...
                <div className="history-card">
                  ...
                  <p className="metric-subtle">
                    {activeHistory.deltaRain > 0
                      ? "Current cycle is drier than the selected reference."
                      : "Current cycle is wetter than the selected reference."}
                  </p>
                </div>
              </div>
            </article>
          ...
          <aside className="stack">
            <article className="glass-card">
              ...
              <div className="card-grid">
                ...
                <div className="metric-card">
                  ...
                  <p className="metric-subtle">
                    Relative station height · {formatDistanceMeters(city.seaLevelM, unit)}
                  </p>
                </div>
                <div className="metric-card">
                  <header>
            ...
            <article className="glass-card">
              ...
              <div className="astro-grid">
                <div className="astro-card">
                  ...
                  <p className="metric-subtle">
                    Sunrise {city.sunrise} to sunset {city.sunset}
                  </p>
                </div>
                <div className="astro-card">
            ...
            <article className="glass-card">
              ...
              <div className="profile-grid">
                <div className="profile-card">
                  ...
                  <p className="metric-value">
                    {data.find((item) => item.id === profile.homeCityId)?.city ?? city.city}
                  </p>
            ...
            <article className="glass-card">
                ...
                {savedViews.length > 0 ? (
                  savedViews.map((entry) => (
                    <button
                      key={entry.id}
                      type="button"
                      className="savedview-card"
                      onClick={() => applySavedView(entry)}
                    >
                      <strong>{entry.name}</strong>
                      <span className="metric-subtle">
                  ...
                  <div className="empty-state">
                    No saved views yet. Save the current dashboard state to reuse it later.
                  </div>
                )}
              </div>
              ...
              <div className="settings-grid">
                ...
                >
                  ...
                  <span className="metric-subtle">
                    {settings.motion ? "Animated fronts and transitions enabled" : "Reduced motion mode enabled"}
                  </span>
                </button>
                <button
                  type="button"
                ...
                >
                  ...
                  <span className="metric-subtle">
                    {settings.denseCards ? "Compact card behavior active" : "Default spacing active"}
                  </span>
                </button>
                <button
                  type="button"
                ...
                >
                  ...
                  <span className="metric-subtle">
                    {settings.advisoryMode ? "Health and trip prep guidance emphasized" : "Guidance layer softened"}
                  </span>
                </button>
              </div>
            </article>
            ...
            <article className="glass-card">
              ...
              <div className="task-list">
                {currentTasks.length > 0 ? (
                  ...
                  <div className="empty-state">
                    No trip tasks yet. Add one manually or load a template for the selected trip purpose.
                  </div>
                )}
              </div>
            </article>
              ...
              <div className="prefs-grid">
                {(["storm", "air", "uv", "commute", "hydration", "travel"] as NotificationType[]).map((type) => (
                  <button key={type} type="button" className="prefs-card" onClick={() => toggleNotificationPref(type)}>
                    ...
                    <span className="metric-subtle">
                      {notificationPrefs[type] ? "Enabled in the alert stream" : "Muted from the alert stream"}
                    </span>
                  </button>
                ))}
              </div>
            ...
            <article className="glass-card">
              ...
              <div className="city-results">
                {filteredCities.length > 0 ? (
                  ...
                  <div className="empty-state">
                    No mock locations match “{query}”. Try Tokyo, Dubai, Reykjavik, or New York.
                  </div>
                )}
              </div>
            </article>
            ...
            <p className="footer-note">
              Single-file build: the full UI, CSS, mock data, React bootstrap, and helper logic live inside
              `src/App.tsx`. It now operates as a large concept project with a timeline, compare mode, planner engine,
              weather map, climate intelligence, astronomy, and an AQI lab.
            </p>
          </aside>
        </section>
      </main>
        ...
        <div className="modal-backdrop" role="dialog" aria-modal="true">
          <div className="modal-panel">
            ...
            <div className="profile-grid">
              ...
              <div className="field">
                ...
                <select
                  ...
                  onChange={(event) => setProfile((current) => ({ ...current, homeCityId: event.target.value }))}
                >
                  {data.map((item) => (
              ...
              <div className="field">
                ...
                <select
                  ...
                  onChange={(event) =>
                    setProfile((current) => ({ ...current, activity: event.target.value as ProfileActivity }))
                  }
                >
              ...
              <div className="field">
                ...
                <input
                  ...
                  onChange={(event) =>
                    setProfile((current) => ({ ...current, heatSensitivity: Number(event.target.value) }))
                  }
                />
              </div>
              <div className="field">
                ...
                <input
                  ...
                  onChange={(event) =>
                    setProfile((current) => ({ ...current, coldSensitivity: Number(event.target.value) }))
                  }
                />
              </div>
            </div>
            ...
            <p className="small-note">
              Profile-aware comfort for {city.city}: {profileBias}. This value shifts recommendations, notifications,
              wardrobe and activity scores.
            </p>
          </div>
        </div>
      ) : null}
          ...
          <div className="modal-panel">
            ...
            <div className="modal-actions">
              <button
                ...
                onClick={() => setEditedWeather((current) => ({ ...current, [city.id]: {} }))}
              >
                Reset city edits
              </button>
