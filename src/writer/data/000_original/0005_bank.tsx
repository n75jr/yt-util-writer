...
const noticeSeed: Notice[] = [
  {
    id: "note-1",
    title: "Travel wallet used in Lisbon",
    detail: "A new device approved a EUR charge 12 minutes ago.",
    tone: "info",
    createdAt: "09:14",
    resolved: false,
  },
  {
    id: "note-2",
    title: "Business card remains frozen",
    detail: "Card ending 8820 is frozen until you unlock it.",
    tone: "warning",
    createdAt: "08:02",
    resolved: false,
  },
  {
    id: "note-3",
    title: "Savings interest posted",
    detail: "A new interest accrual of $126.44 has been added.",
    tone: "success",
    createdAt: "07:45",
    resolved: false,
  },
  {
    id: "note-4",
    title: "Large debit needs review",
    detail: "One scheduled vendor payment exceeds your normal range.",
    tone: "critical",
    createdAt: "06:21",
    resolved: false,
  },
...
const supportSeed: Message[] = [
  {
    id: "msg-1",
    from: "bank",
    text: "Northstar concierge here. I can help with transfers, cards, and branch appointments.",
    createdAt: "09:00",
  },
...
const faqItems = [
  {
    ...
    a: "Open the Cards tab and use Freeze card. You can unlock it later without replacing the plastic.",
  },
  {
    q: "Can I move money between currencies?",
    a: "Yes. The FX panel converts the amount and posts a simulated transfer into your selected destination account.",
  },
  {
    q: "What happens when I snooze a bill?",
    a: "The due date shifts by three days and the bill gets a snoozed status until you pay it.",
  },
  {
    q: "How is loan estimate calculated?",
    a: "This demo uses a simple amortization formula with the rate and term you pick.",
  },
];
...
function App() {
  ...
  const filteredTransactions = useMemo(() => {
    ...
    return transactions
      ...
      .filter((txn) => {
        ...
        return [txn.title, txn.merchant, txn.category, txn.note, txn.tag].some((value) =>
  ...
  const monthlyInflow = useMemo(
    () => transactions.filter((txn) => txn.direction === "credit").reduce((sum, txn) => sum + txn.amount, 0),
    [transactions],
  );
  const monthlyOutflow = useMemo(
    () => transactions.filter((txn) => txn.direction === "debit").reduce((sum, txn) => sum + txn.amount, 0),
    [transactions],
  );
  const primaryAccount = useMemo(
    () => accounts.find((account) => account.id === "acc-main") ?? accounts[0],
    [accounts],
  );
  const dueBills = useMemo(() => bills.filter((bill) => bill.status !== "paid").slice(0, 3), [bills]);
  const personalFeed = useMemo(
    () => transactions.filter((txn) => txn.accountId === "acc-main").slice(0, 4),
    [transactions],
  );
  ...
  const loanMonthly = useMemo(
    () => calculateLoan(Number(loanAmount || 0), Number(loanRate || 0), Number(loanYears || 1)),
    ...
    setAccounts((current) =>
      current.map((account) =>
        account.id === accountId
          ? {
              ...account,
              balance: Number((account.balance + deltaBalance).toFixed(2)),
              available: Number((account.available + deltaAvailable).toFixed(2)),
            }
          : account,
      ),
    );
  ...
  function simulateSalary() {
    ...
    postTransaction({
      ...
      status: "booked",
      date: "2026-03-14",
      note: "Manual demo salary trigger",
      tag: "salary-demo",
    });
  ...
  function simulateExpense() {
    ...
    postTransaction({
      accountId: "acc-main",
      title: "Cafe expense",
      merchant: "Signal Cafe",
      amount,
      direction: "debit",
      category: "Food",
      status: "booked",
      date: "2026-03-14",
      note: "Quick expense simulation",
      tag: "coffee-demo",
    });
    setCards((current) =>
      current.map((card) =>
        card.id === "card-1" ? { ...card, spent: Number((card.spent + amount).toFixed(2)) } : card,
      ),
    );
  ...
  function submitTransfer() {
    ...
    postTransaction({
      ...
      merchant: "Northstar Internal",
      amount,
      direction: "debit",
      category: "Bills",
      status: "booked",
      date: "2026-03-14",
      note: transferForm.memo || "Internal transfer",
      tag: "transfer-out",
    });
    postTransaction({
    ...
    addNotice(
      "Transfer completed",
      `${formatMoney(amount, fromAccount.currency)} moved to ${toAccount.name}.`,
      "success",
    );
    logAction(
      `Transferred ${formatMoney(amount, fromAccount.currency)} from ${fromAccount.name} to ${toAccount.name}.`,
    );
  }
  ...
  function submitPayment() {
    ...
    postTransaction({
      ...
      title: paymentForm.payee,
      merchant: paymentForm.payee,
      amount,
      direction: "debit",
      category: "Bills",
      status: "booked",
      date: "2026-03-14",
      note: paymentForm.memo || "Manual bill pay",
      tag: "bill-pay",
    });
  ...
  function snoozeBill(id: string) {
    setBills((current) =>
      current.map((entry) => (entry.id === id ? { ...entry, due: "2026-04-01", status: "snoozed" } : entry)),
    );
  ...
  function rewardInterest(accountId: string) {
    ...
    postTransaction({
      ...
      merchant: "Northstar Reserve",
      amount,
      direction: "credit",
      category: "Investing",
      status: "booked",
      date: "2026-03-14",
      note: "Manual interest simulation",
      tag: "interest",
    });
  ...
  function toggleFreeze(cardId: string) {
    ...
    if (card) {
      addNotice(
        card.frozen ? "Card unlocked" : "Card frozen",
        `${card.label} ending ${card.last4} changed status.`,
        card.frozen ? "success" : "warning",
      );
    }
  ...
  function replaceCard(cardId: string) {
    setCards((current) =>
      current.map((card) =>
        card.id === cardId
          ? { ...card, last4: String(Math.floor(1000 + Math.random() * 9000)), frozen: false, spent: 0 }
          : card,
      ),
    );
  ...
  function simulateCardSpend(cardId: string) {
    ...
    setCards((current) =>
      current.map((entry) =>
        entry.id === cardId ? { ...entry, spent: Number((entry.spent + amount).toFixed(2)) } : entry,
      ),
    );
    adjustAccount(card.accountId, -amount);
    postTransaction({
      accountId: card.accountId,
      title: `${card.label} purchase`,
      merchant: "Terminal Atelier",
      amount,
      direction: "debit",
      category: "Shopping",
      status: "booked",
      date: "2026-03-14",
      note: "Simulated card transaction",
      tag: "card-sim",
    });
  ...
  function contributeGoal(goalId: string, amount: number) {
    ...
    setGoals((current) =>
      current.map((entry) =>
        entry.id === goalId ? { ...entry, current: clamp(entry.current + amount, 0, entry.target) } : entry,
      ),
    );
    postTransaction({
      accountId: source.id,
      title: `Goal contribution: ${goal.name}`,
      merchant: "Northstar Vault",
      amount,
      direction: "debit",
      category: "Investing",
      status: "booked",
      date: "2026-03-14",
      note: "Vault contribution",
      tag: "goal",
    });
  ...
  function trimSubscriptions() {
    ...
    postTransaction({
      ...
      merchant: "Northstar Optimizer",
      amount,
      direction: "credit",
      category: "Bills",
      status: "booked",
      date: "2026-03-14",
      note: "Recovered from recurring spend review",
      tag: "optimizer",
    });
  ...
  function exchangeFunds() {
    ...
    postTransaction({
      ...
      merchant: "Northstar FX",
      amount,
      direction: "debit",
      category: "Travel",
      status: "booked",
      date: "2026-03-14",
      note: `Converted at ${rate}`,
      tag: "fx-out",
    });
    postTransaction({
      ...
      status: "booked",
      date: "2026-03-14",
      note: `Converted from USD at ${rate}`,
      tag: "fx-in",
    });
        ...
        <section className="top-grid">
          <article className="hero-panel">
            ...
            <p className="hero-copy">
              Личный кабинет с актуальным балансом, быстрыми платежами, картами и движением денег. Это уже ближе к
              реальному интерфейсу современного банковского приложения, а не к абстрактному дашборду.
            </p>
            <div className="personal-shell">
              <div>
                <div className="balance-card">
                  ...
                  <div className="muted">
                    Available {formatMoney(primaryAccount.available, primaryAccount.currency)} on {primaryAccount.name}
                  </div>
                ...
                <div className="quick-actions-grid">
                  ...
                  <button
                    ...
                    onClick={() => quickPay("Harbor Rent", 1800, "Monthly rent payment")}
                  >
                  ...
                  <button
                    className="shortcut-btn"
                    onClick={() => quickPay("Luna Wireless", 91.74, "Mobile plan", "Bills")}
                  >
                ...
                <div className="mini-metrics">
                  ...
                  <div className="mini-metric">
                    ...
                    <div className="positive" style={{ fontSize: 24, marginTop: 6 }}>
                      {formatMoney(monthlyInflow)}
                    </div>
                  </div>
                  <div className="mini-metric">
                    <div className="metric-label">Expenses</div>
                    <div className="negative" style={{ fontSize: 24, marginTop: 6 }}>
              ...
              <div className="mini-stack">
                  ...
                  <div className="list-rows">
                    {dueBills.map((bill) => (
                      <div key={bill.id} className="pay-row">
                        ...
                        <button className="btn btn-primary" onClick={() => payBill(bill.id)}>
                          Pay {formatMoney(bill.amount)}
                        </button>
                      </div>
                    ))}
                  </div>
                ...
                <div className="mini-card">
                  <div className="split-header">
                    ...
                    <button className="btn btn-secondary" onClick={() => selectAccount("acc-main")}>
                      Open feed
                    </button>
                  </div>
                    ...
                    {personalFeed.map((txn) => (
                      <div key={`cabinet-${txn.id}`} className="cabinet-item">
                        <span
                          ...
                          style={{ background: txn.direction === "credit" ? "#70e4c2" : "#5b8def" }}
                        />
                        <div>
                          <div>{txn.title}</div>
          ...
          <aside className="side-panel">
            <div className="split-header">
              ...
              <div className="toolbar">
                ...
                <button className="btn btn-secondary" onClick={toggleTheme}>
                  {theme === "obsidian" ? "Light UI" : "Dark UI"}
                </button>
              </div>
            </div>
            <div className="mini-card" style={{ marginBottom: 14 }}>
              ...
              <div style={{ fontSize: 26, letterSpacing: "0.16em", marginBottom: 8 }}>
                •••• •••• •••• {selectedCard.last4}
              </div>
              <div className="muted">
                {selectedCard.holder} • {selectedCard.network}
              </div>
              <div className="mini-metrics">
                <div className="mini-metric">
              ...
              <div className="toolbar" style={{ marginTop: 14 }}>
                <button className="btn btn-secondary" onClick={() => toggleFreeze(selectedCard.id)}>
                  {selectedCard.frozen ? "Unlock" : "Freeze"}
                </button>
                <button className="btn btn-primary" onClick={() => simulateCardSpend(selectedCard.id)}>
                  Pay with card
                </button>
                <button className="btn btn-secondary" onClick={clearResolvedNotices}>
                  Clear alerts
                </button>
              </div>
            </div>
            <div className="ledger-list">
              ...
              <div className="ledger-row">
                ...
                <button className="btn btn-secondary" onClick={simulateSalary}>
                  Run
                </button>
              </div>
              <div className="ledger-row">
                ...
                <button className="btn btn-secondary" onClick={simulateExpense}>
                  Run
                </button>
              </div>
            </div>
            ...
            {activeTab === "overview" && (
              <>
                <div className="split-header">
                  ...
                  <div className="toolbar">
                    <button className="btn btn-secondary" onClick={() => selectAccount("all")}>
                      All accounts
                    </button>
                    <button className="btn btn-secondary" onClick={() => setActiveTab("support")}>
                      Open support
                    </button>
                    <button className="btn btn-primary" onClick={rebalanceSavings}>
                      Rebalance savings
                    </button>
                  </div>
                </div>
                <div className="stats-row">
                  ...
                  <article className="stat-card">
                    ...
                    <div className="metric-value">
                      {formatCompact((totals.totalCardSpend / cards.reduce((sum, card) => sum + card.limit, 0)) * 100)}%
                    </div>
                  ...
                  {notices.slice(0, 4).map((notice) => (
                    <article key={notice.id} className="notice-card">
                      ...
                      <div className="notice-actions">
                        <button className="btn btn-secondary" onClick={() => resolveNotice(notice.id)}>
                          Mark reviewed
                        </button>
                        <button className="btn btn-secondary" onClick={() => setActiveTab("support")}>
                          Escalate
                        </button>
                        <button className="btn btn-primary" onClick={() => selectAccount("acc-main")}>
                          Focus ledger
                        </button>
                      </div>
                    </article>
              ...
              <>
                <div className="split-header">
                  <div>
                    ...
                    <p className="muted">
                      Each account card can post demo interest, focus the ledger, or prepare a statement preview.
                    </p>
                  </div>
                  <div className="toolbar">
                    <button className="btn btn-secondary" onClick={() => selectAccount("all")}>
                    ...
                    <button className="btn btn-primary" onClick={() => rewardInterest("acc-save")}>
                ...
                <div className="account-grid">
                  {accounts.map((account) => (
                    <article key={account.id} className="account-card" style={{ borderColor: `${account.accent}55` }}>
                      ...
                      <div className="muted">
                        Available {formatMoney(account.available, account.currency)} • Rate {account.rate}% APY
                      </div>
                      <div className="sparkline">
                        {account.analytics.map((value, index) => (
                          <span
                            key={`${account.id}-${index}`}
                            style={{
                              height: `${value}%`,
                              background: `linear-gradient(180deg, ${account.accent}, rgba(255,255,255,0.12))`,
                            }}
                          />
                        ))}
                      </div>
                      ...
                      <div className="toolbar">
                        <button className="btn btn-secondary" onClick={() => selectAccount(account.id)}>
                          Focus ledger
                        </button>
                        <button className="btn btn-primary" onClick={() => rewardInterest(account.id)}>
                          Add interest
                        </button>
                        <button className="btn btn-accent" onClick={() => exportStatement(account.id)}>
                          Prepare statement
                        </button>
                      </div>
                    </article>
              ...
              <>
                <div className="split-header">
                  <div>
                    ...
                    <p className="muted">
                      Presets load forms instantly, then buttons post real debits and credits into the ledger.
                    </p>
                  </div>
                </div>
                <div className="form-grid">
                    ...
                    <div className="split-header">
                      ...
                      <div className="toolbar">
                        {quickTransferPresets.map((preset, index) => (
                          <button key={preset.label} className="btn btn-secondary" onClick={() => applyPreset(index)}>
                            {preset.label}
                          </button>
                        ))}
                      </div>
                    ...
                    <div className="form-grid">
                      <div className="field">
                        ...
                        <select
                          ...
                          onChange={(event) => setTransferForm((current) => ({ ...current, from: event.target.value }))}
                        >
                          {accounts.map((account) => (
                      ...
                      <div className="field">
                        ...
                        <input
                          ...
                          onChange={(event) => setTransferForm((current) => ({ ...current, memo: event.target.value }))}
                        />
                      </div>
                    </div>
                  ...
                  <article className="feed-card" style={{ padding: 22 }}>
                    ...
                    <div className="form-grid">
                      <div className="field">
                        ...
                        <select
                          ...
                          onChange={(event) => setPaymentForm((current) => ({ ...current, payee: event.target.value }))}
                        >
                          {payees.map((payee) => (
                      ...
                      <div className="field">
                        ...
                        <select
                          ...
                          onChange={(event) =>
                            setPaymentForm((current) => ({ ...current, accountId: event.target.value }))
                          }
                        >
                          {accounts.map((account) => (
                            <option key={account.id} value={account.id}>
                    ...
                    <div className="toolbar" style={{ marginTop: 16 }}>
                      <button className="btn btn-accent" onClick={submitPayment}>
                        Pay now
                      </button>
                ...
                <div className="bill-grid" style={{ marginTop: 16 }}>
                  {bills.map((bill) => (
                    <article key={bill.id} className="bill-card">
                      ...
                      <p className="muted">
                        Due {bill.due} • Funding account{" "}
                        {accounts.find((account) => account.id === bill.accountId)?.name}
                      </p>
                      <div className="bill-actions">
                        <button className="btn btn-primary" onClick={() => payBill(bill.id)}>
                          Pay bill
                        </button>
                        <button className="btn btn-secondary" onClick={() => snoozeBill(bill.id)}>
                          Snooze
                        </button>
                        <button className="btn btn-secondary" onClick={() => toggleAutopay(bill.id)}>
              ...
              <>
                <div className="split-header">
                  <div>
                    ...
                    <p className="muted">
                      Lock, replace, raise limits, and simulate real spend. All actions update the balance layer.
                    </p>
                  </div>
                  <div className="toolbar">
                    <button className="btn btn-secondary" onClick={() => setSelectedCardId("card-1")}>
                      Primary card
                    </button>
                    <button className="btn btn-primary" onClick={() => setSelectedCardId("card-3")}>
                      Business card
                    </button>
                  </div>
                </div>
                  ...
                  {cards.map((card) => (
                    <article key={card.id} className="card-shell" style={{ borderColor: `${card.metal}44` }}>
                      <div className="split-header">
                        ...
                        <span className="pill" style={{ background: card.metal, color: "#111" }}>
                      ...
                      <div className="metrics">
                        <div>
                          <div className="metric-label">Limit</div>
                          <div>{formatMoney(card.limit)}</div>
                        </div>
                        <div>
                          <div className="metric-label">Spent</div>
                          <div>{formatMoney(card.spent)}</div>
                        </div>
                        <div>
                          <div className="metric-label">Left</div>
                          <div>{formatMoney(card.limit - card.spent)}</div>
                        </div>
                      </div>
                      <div className="card-actions" style={{ marginTop: 16 }}>
                        <button className="btn btn-secondary" onClick={() => toggleFreeze(card.id)}>
                          {card.frozen ? "Unlock" : "Freeze card"}
                        </button>
                        <button className="btn btn-primary" onClick={() => raiseLimit(card.id)}>
                          Raise limit
                        </button>
                        <button className="btn btn-accent" onClick={() => simulateCardSpend(card.id)}>
                          Simulate spend
                        </button>
                        <button className="btn btn-gold" onClick={() => replaceCard(card.id)}>
                          Replace
                        </button>
                      </div>
                    </article>
                ...
                <article className="feed-card" style={{ padding: 22, marginTop: 16 }}>
                  <div className="split-header">
                    ...
                    <button className="btn btn-secondary" onClick={() => simulateCardSpend(selectedCard.id)}>
                      One more purchase
                    </button>
                  </div>
              ...
              <>
                <div className="split-header">
                  <div>
                    ...
                    <p className="muted">
                      Move money into goals, change monthly pace, and simulate a currency exchange for travel.
                    </p>
                  </div>
                  <div className="toolbar">
                    <button className="btn btn-primary" onClick={() => contributeGoal("goal-1", 90)}>
                      Top up trip
                    </button>
                    <button className="btn btn-secondary" onClick={exchangeFunds}>
                      Run FX conversion
                    </button>
                  </div>
                </div>
                    ...
                    return (
                      <article key={goal.id} className="goal-card">
                        <div className="split-header">
                          ...
                          <div className="ring" style={{ ["--ring" as string]: `${progress}%` }}>
                            <span>{progress}%</span>
                          </div>
                        </div>
                        <p className="muted">
                          {formatMoney(goal.current)} of {formatMoney(goal.target)} • Monthly pace{" "}
                          {formatMoney(goal.pace)}
                        </p>
                        <div className="goal-actions">
                          <button className="btn btn-primary" onClick={() => contributeGoal(goal.id, 75)}>
                            Contribute
                          </button>
                          <button className="btn btn-secondary" onClick={() => boostGoal(goal.id)}>
                            Boost pace
                          </button>
                          <button className="btn btn-accent" onClick={() => toggleGoalPause(goal.id)}>
                ...
                <article className="feed-card" style={{ padding: 22, marginTop: 16 }}>
                  ...
                  <div className="toolbar" style={{ marginTop: 16 }}>
                    <button className="btn btn-primary" onClick={exchangeFunds}>
                      Exchange now
                    </button>
                    <button
                      className="btn btn-secondary"
                      onClick={() => {
                        setFxAmount("500");
                        setFxRate("0.94");
                        logAction("FX preset adjusted for a stronger euro scenario.");
                      }}
                    >
                      Use stronger EUR rate
                    </button>
                  </div>
                </article>
              </>
              ...
              <>
                <div className="split-header">
                  <div>
                    ...
                    <p className="muted">
                      Category bars summarize recent transaction volume while the loan block recalculates instantly.
                    </p>
                  </div>
                  <div className="insight-actions">
                    <button className="btn btn-primary" onClick={trimSubscriptions}>
                      Trim subscriptions
                    </button>
                    <button className="btn btn-secondary" onClick={refreshMarket}>
                      Refresh assumptions
                    </button>
                  </div>
                </div>
                <article className="feed-card" style={{ padding: 22 }}>
                  ...
                  <div className="insight-actions" style={{ marginTop: 18 }}>
                    <button className="btn btn-secondary" onClick={() => selectAccount("acc-business")}>
                      Focus business spend
                    </button>
                    <button className="btn btn-secondary" onClick={() => selectAccount("acc-main")}>
                      Focus personal spend
                    </button>
                  </div>
                </article>
                <article className="feed-card" style={{ padding: 22, marginTop: 16 }}>
                  ...
                  <div className="form-grid">
                    ...
                    <div className="field">
                      ...
                      <input value={loanRate} onChange={(event) => setLoanRate(event.target.value)} />
                    </div>
                    <div className="field">
                      <label>Years</label>
                      <input value={loanYears} onChange={(event) => setLoanYears(event.target.value)} />
                    </div>
                    <div className="field">
                      <label>Scenario notes</label>
                      <textarea
                        value={`Estimated repayment over ${loanYears} years at ${loanRate}%`}
                        readOnly
                        rows={3}
                      />
                    </div>
                  ...
                  <div className="insight-actions" style={{ marginTop: 16 }}>
                    <button
                      ...
                      onClick={() => {
                        ...
                        setLoanRate("6.9");
                        setLoanYears("5");
                        logAction("Loan scenario switched to medium-term plan.");
                      }}
                    >
                      Load medium plan
                    </button>
                    <button
                      ...
                      onClick={() => {
                        ...
                        setLoanRate("5.2");
                        setLoanYears("2");
                        logAction("Loan scenario switched to short-term plan.");
                      }}
                    >
                      Load short plan
                    </button>
              ...
              <>
                <div className="split-header">
                  <div>
                    ...
                    <p className="muted">
                      Chat, FAQ injections, and branch slots all perform stateful changes inside the app.
                    </p>
                  </div>
                </div>
                <div className="form-grid">
                  <article className="chat-card">
                    ...
                    <div className="support-actions" style={{ marginTop: 14 }}>
                      <button className="btn btn-primary" onClick={() => sendSupportMessage(supportDraft)}>
                        Send message
                      </button>
                      {cannedSupportReplies.map((reply) => (
                        <button key={reply} className="btn btn-secondary" onClick={() => sendSupportMessage(reply)}>
                          {reply.slice(0, 18)}...
                        </button>
                      ))}
                    </div>
                    ...
                    <div className="ledger-list">
                      {branchSlots.map((slot) => (
                        <div key={slot.id} className="ledger-row">
                          ...
                          <button className="btn btn-secondary" onClick={() => bookBranchSlot(slot.id)}>
                  ...
                  {faqItems.map((faq) => (
                    <article key={faq.q} className="faq-card">
                      ...
                      <div className="faq-actions">
                        <button className="btn btn-primary" onClick={() => injectFaqAnswer(faq.a)}>
                          Send to chat
                        </button>
                        <button className="btn btn-secondary" onClick={() => setSupportDraft(faq.q)}>
                          Use as draft
                        </button>
                      </div>
                    </article>
          ...
          <aside className="side-panel">
            <div className="split-header">
              ...
              <div className="cluster">
                ...
                <button className="btn btn-secondary" onClick={() => selectAccount("acc-save")}>
                  Savings
                </button>
              </div>
            </div>
            ...
            <div className="ledger-list">
              {filteredTransactions.map((txn) => (
                <div key={txn.id} className="ledger-row">
                  <span
                    className="avatar-dot"
                    style={{ background: txn.direction === "credit" ? "#9fd7b5" : "#f2c6c2" }}
                  />
                  <div>
                    <div>{txn.title}</div>
                    <div className="muted">
                      {txn.date} • {txn.merchant} • {txn.note}
                    </div>
                  </div>
                  <strong className={txn.direction === "credit" ? "positive" : "negative"}>
                    {txn.direction === "credit" ? "+" : "-"}
            ...
            <div className="split-header" style={{ marginTop: 20 }}>
              ...
              <button className="btn btn-secondary" onClick={() => exportStatement(statementAccountId)}>
                Refresh statement
              </button>
            </div>
            <div className="activity-list">
              {transactions
                .filter((txn) => txn.accountId === statementAccountId)
                .slice(0, 8)
                .map((txn) => (
            ...
            <div className="split-header" style={{ marginTop: 20 }}>
              ...
              <button className="btn btn-secondary" onClick={() => setActivity([])}>
                Clear log
              </button>
            </div>
            <div className="activity-list">
            ...
            <p className="footer-note">
              All state is stored locally in the browser for this demo. Reloading keeps your simulated banking session
              intact.
            </p>
          </aside>
        </section>
